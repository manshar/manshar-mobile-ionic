...
angular.module('manshar.controllers')
  .controller('EditProfileCtrl', ['$scope', '$rootScope', '$location','$state', '$stateParams', 'User', '$window', 'Link', 'UserLink',
      function ($scope, $rootScope, $location,$state, $stateParams, User,  $window, Link, UserLink) {
    ...
    var authorizeUser = function (user) {
      ...
      if (id !== user.id) {
        $state.go('app.profile',{'userId': $stateParams.userId})
      }
    };

    $scope.profile = User.get({'userId': $stateParams.userId}, authorizeUser);
    $scope.error = null;
    $scope.errorMessages = {};
    ...
    var updateUserDataSuccess = function() {
     ...
     // alert()
      $state.go('app.profile',{'userId': $stateParams.userId})
    //  $location.path('#/app/profiles/' + $stateParams.userId);
    };
    ...
    $scope.createLink = function(link) {
      ...
      link.userId = $stateParams.userId;
